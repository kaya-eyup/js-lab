...
export class HttpError extends Error {
  ...
  get userMessage() {
    ...
    if (this.status >= 500)
      return "Sunucu kaynaklı bir sorun oluştu, lütfen sonra tekrar deneyin.";
    return "Beklenmeyen bir ağ hatası oluştu.";
  }
}

// export edilecek fetch fonksiyonumuz
export function http(url: string, options: RequestInit = {}): Promise<unknown> {
  // any dönmemesi için
  return fetch(url, options).then((response) => {
    ...
    return response.text().then((text) => {
      throw new Error(
        `Beklenen JSON yerine farklı tip geldi (${contentType}): ${text.slice(0, 100)}`,
      );
    });
  });
}
